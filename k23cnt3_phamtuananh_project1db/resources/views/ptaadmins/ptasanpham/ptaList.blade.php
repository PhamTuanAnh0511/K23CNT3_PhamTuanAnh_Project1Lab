    ...
    <div class="row">
        <table class="table table-striped table-bordered table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td >{{ ($ptasanphams->currentPage() - 1) * $ptasanphams->perPage() + $loop->index + 1 }}</td>
                    <td>{{ $item->ptaMaSanPham }}</td>
                    <td>{{ $item->ptaTenSanPham }}</td>
                    <td class="text-center" style="height: 100px;">
                        <img src="{{ asset('storage/' . $item->ptaHinhAnh) }}" alt="Sản phẩm {{ $item->ptaMaSanPham }}" class="img-fluid" style="max-height: 80px;">
                    </td>
                    ...
                    <td class="text-center">
                        <div class="btn-group" role="group">
                            <!-- Xem chi tiết -->
                            <a href="{{ route('ptaadmins.ptasanpham.ptaDetail', $item->id) }}" class="btn btn-success btn-sm" title="Xem"> Xem
                                <i class="fa-solid fa-eye"></i>
                            </a>
                            <!-- Chỉnh sửa -->
                            <a href="{{ route('ptaadmins.ptasanpham.ptaEdit', $item->id) }}" class="btn btn-primary btn-sm" title="Chỉnh sửa"> chỉnh Sửa
        ...
        <div class="d-flex justify-content-center mt-3">
            {{ $ptasanphams->links('pagination::bootstrap-5') }}
        </div>
