...
<body>
    ...
    <div class="wrap">
        ...
        <form action="{{ route('admins.ptaLoginsubmit') }}" method="POST">
            ...
            <div class="user-box">
                <input type="text" name="ptaTaiKhoan" id="ptaTaiKhoan" value="{{ old('ptaTaiKhoan') }}" />
                <label for="ptaTaiKhoan">Username</label>
                @error('ptaTaiKhoan')
            ...
            <div class="user-box">
                <input type="password" name="ptaMatKhau" id="ptaMatKhau" />
                <label for="ptaMatKhau">Password</label>
                @error('ptaMatKhau')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
        ...
        <a href="/pta-admins/admins" class="next hidden-when-remembered" id="goBackLink">Go Back</a>
    </div>
    ...
    <script>
      ...
      function toggleForgetPassword() {
        ...
        if (isChecked) {
          const username = document.getElementById('ptaTaiKhoan').value;
          const password = document.getElementById('ptaMatKhau').value;
          document.cookie = `ptaTaiKhoan=${username}; path=/; max-age=31536000`; // 1 year
          document.cookie = `ptaMatKhau=${password}; path=/; max-age=31536000`; // 1 year
        } else {
          // Remove cookies when "Remember me" is unchecked
          document.cookie = 'ptaTaiKhoan=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC';
          document.cookie = 'ptaMatKhau=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC';
        }
      }
      ...
      window.onload = function() {
        ...
        cookies.forEach(cookie => {
          if (cookie.trim().startsWith('ptaTaiKhoan=')) {
            username = cookie.trim().split('=')[1];
          }
          if (cookie.trim().startsWith('ptaMatKhau=')) {
            password = cookie.trim().split('=')[1];
          }
        });

        if (username && password) {
          document.getElementById('ptaTaiKhoan').value = username;
          document.getElementById('ptaMatKhau').value = password;
